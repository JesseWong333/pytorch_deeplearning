...
def predict(net, img_path, save_path):
    img = cv2.imread(img_path)
    img, angle = correct_image(img)
    file_name = img_path.split('/')[-1].split('.')[0]
    ...
    with open(os.path.join(save_path, file_name + '.txt'), 'w') as f_txt:
        for cord in cords:
            f_txt.write(','.join(map(lambda x: str(int(x)), cord)) + '\n')
...
if __name__ == '__main__':
    ...
    state_dict = torch.load('/media/Data/wangjunjie_code/pytorch_text_detection/checkpoints/pixel_based_OHEM2/35_net_net.pth')
    ...
    folder = "/media/Data/wangjunjie_code/advancedEast/dataset/"
    save_path_root = '/media/Data/wangjunjie_code/pytorch_text_detection/OKM'

    subjects = ['junior_biology', 'junior_chemistry', 'junior_geography', 'junior_history', 'junior_math',
                'junior_physics', 'junior_politics', 'senior_biology', 'senior_chemistry', 'senior_math',
                'senior_physics']

    for subject in subjects:
        img_path_list = glob.glob(os.path.join(folder, subject, '*.png'))
        save_path = os.path.join(save_path_root, subject)

        if not os.path.exists(save_path):
            os.makedirs(save_path)

        for img_path in tqdm(img_path_list):
            predict(net, img_path, save_path)