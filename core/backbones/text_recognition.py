...
class DenseNet(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(input_channel, num_init_features, kernel_size=7, stride=2,
        ...
        # Final batch norm
        self.features.add_module('block5', nn.Sequential(OrderedDict([
                   ('norm5', nn.BatchNorm2d(num_features)),
                   ('relu5', nn.ReLU(inplace=True))])))

        self.features.add_module('block6', nn.Sequential(OrderedDict([
                   ('conv6', nn.Conv2d(num_features, output_channel, kernel_size=1, stride=1,
                                bias=False)),
                   ('norm6', nn.BatchNorm2d(output_channel))])))
            # ('relu5', nn.ReLU(inplace=True)),
...
def _densenet(arch, input_channel, output_channel, \
              growth_rate, block_config, num_init_features,
              **kwargs):
    model = DenseNet(input_channel, output_channel, growth_rate, block_config, num_init_features, **kwargs)
    return model


def densenet161(input_channel, output_channel, **kwargs):
    ...
    Args:
        ...
        memory_efficient (bool) - If True, uses checkpointing. Much more memory efficient,
          but slower. Default: *False*. See `"paper" <https://arxiv.org/pdf/1707.06990.pdf>`_
    """
    return _densenet('densenet161', input_channel, output_channel, 48, (6, 12, 36, 24), 96,
                     **kwargs)


def densenet121(input_channel, output_channel, **kwargs):
    r"""Densenet-161 model from
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`_
    ...
    Args:
        ...
        memory_efficient (bool) - If True, uses checkpointing. Much more memory efficient,
          but slower. Default: *False*. See `"paper" <https://arxiv.org/pdf/1707.06990.pdf>`_
    """
    return _densenet('densenet121', input_channel, output_channel, 32, (6, 12, 24, 16), 64,
                     **kwargs)
...
class Densenet_FeatureExtractor(nn.Module):
    ...
    def __init__(self, input_channel, output_channel=512):
        ...
        self.feature = densenet121(input_channel, output_channel)
...
class RCNN_FeatureExtractor(nn.Module):
    ...
    def __init__(self, input_channel, output_channel=512, num_iteration=5):
        ...
        self.ConvNet = nn.Sequential(
            ...
            # nn.MaxPool2d(2, 2),  # 64 x 16 x 50
            GRCL(self.output_channel[0], self.output_channel[0], num_iteration=num_iteration, kernel_size=3, pad=1),
            nn.MaxPool2d(2, 2),  # 64 x 8 x 25
            GRCL(self.output_channel[0], self.output_channel[1], num_iteration=num_iteration, kernel_size=3, pad=1),
            nn.MaxPool2d(2, (2, 1), (0, 1)),  # 128 x 4 x 26
            GRCL(self.output_channel[1], self.output_channel[2], num_iteration=num_iteration, kernel_size=3, pad=1),
...
#
#     def __init__(self, input_size, hidden_size, output_size, require_linear=True, batch_first=True, dropout=0):
...
class BidirectionalResLSTM(nn.Module):
    ...
    def __init__(self, num_layers, input_size, rnn_size, dropout):
        ...
        for _ in range(num_layers):
            self.layers.append(nn.LSTM(input_size, rnn_size, bidirectional=True, batch_first=True))
            # input_size = rnn_size
...
class TextRecognition(nn.Module):
    ...
    def __init__(self, FeatureExtraction, SequenceModeling, Prediction, num_class, **kwargs):
        super(TextRecognition, self).__init__()
        # self.cfg = cfg
        self.stages = {'Feat': FeatureExtraction.type,
                       'Seq': SequenceModeling.type, 'Pred': Prediction.type}

        """ FeatureExtraction """
        if FeatureExtraction.type == 'VGG':
            self.FeatureExtraction = VGG_FeatureExtractor(FeatureExtraction.in_channels, \
                                                          FeatureExtraction.out_channels)
        elif FeatureExtraction.type == 'RCNN':
            self.FeatureExtraction = RCNN_FeatureExtractor(FeatureExtraction.in_channels, \
                                                           FeatureExtraction.out_channels,
                                                           FeatureExtraction.num_iteration)
        elif FeatureExtraction.type == 'ResNet':
            self.FeatureExtraction = ResNet_FeatureExtractor(FeatureExtraction.in_channels, \
                                                             FeatureExtraction.out_channels)
        elif FeatureExtraction.type == 'Densenet':
            self.FeatureExtraction = Densenet_FeatureExtractor(FeatureExtraction.in_channels, \
                                                               FeatureExtraction.out_channels)
        else:
            raise Exception('No FeatureExtraction module specified')
        self.FeatureExtraction_output = FeatureExtraction.out_channels  # int(imgH/16-1) * 512
        # self.AdaptiveAvgPool = nn.AdaptiveAvgPool2d((None, 1))  # Transform final (imgH/16-1) -> 1

        """ Sequence modeling"""
        if SequenceModeling.type == 'BiLSTM':
            self.SequenceModeling = nn.Sequential(
                BidirectionalLSTM(self.FeatureExtraction_output, SequenceModeling.hidden_size, \
                                  SequenceModeling.batch_first, SequenceModeling.dropout),
                nn.Linear(SequenceModeling.hidden_size * 2, SequenceModeling.hidden_size),
                BidirectionalLSTM(SequenceModeling.hidden_size, SequenceModeling.hidden_size, \
                                  SequenceModeling.batch_first, SequenceModeling.dropout))
            self.SequenceModeling_output = SequenceModeling.hidden_size * 2
        elif SequenceModeling.type == 'ResBiLSTM':
            self.SequenceModeling = BidirectionalResLSTM(SequenceModeling.layers, \
                                                              self.FeatureExtraction_output,
                                                              SequenceModeling.hidden_size, \
                                                              SequenceModeling.dropout)
            self.SequenceModeling_output = SequenceModeling.hidden_size * 2

        else:
            # print('No SequenceModeling module specified')
            self.SequenceModeling_output = self.FeatureExtraction_output
        self.SequenceModeling_batch_first = SequenceModeling.batch_first

        """ Prediction """
        if Prediction.type == 'CTC':
            self.Prediction = nn.Linear(self.SequenceModeling_output, num_class)
        elif Prediction.type == 'Attn':
            self.Prediction = Attention(self.SequenceModeling_output, SequenceModeling.hidden_size, num_class)
        # else:
    ...
    def forward(self, input, text=None, is_train=True):
        ...
        if self.SequenceModeling_batch_first:
            ...
            visual_feature = visual_feature.permute(2, 0, 1)
        ...
        if self.stages['Pred'] == 'CTC':
            ...
            # prediction = prediction.log_softmax(2)
